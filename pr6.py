...
def build_report_all():
    ...
    md.append("Доля ложноположительных для t=1..7 (ось Y логарифмическая).\n")
    uri = to_data_uri(FILES / "mr_false_rate.png")
    if uri:
        md.append(
            md_small("Что показывает график и почему так")
        )
        md.append(
            "Ось X — число раундов t, ось Y — доля ложноположительных (лог-шкала). "
            "Для t=1 мы наблюдаем ≈2.4×10⁻³ — это **намного ниже** теоретической худшей оценки 1/4. "
            "Причина проста: у большинства составных чисел почти нет «сильных лжецов» (оснований, на которых они проходят MR), "
            "а числа Кармайкла в диапазоне редки. Поэтому уже первый раунд ловит почти всё.\n"
            "Линейность на лог-шкале означает почти экспоненциальное падение ошибки при добавлении раундов, что согласуется с оценкой ≤(1/4)^t. "
            "Именно поэтому прибавка 1–2 раундов даёт резкий прирост надёжности; на нашем датасете для t≥2–3 ошибка часто падает ниже разрешения выборки, "
            "и точки/полосы на графике просто исчезают. На практике 3–5 раундов дают запас надёжности с большим запасом.\n"
        )
        md.append(f"![]({uri})\n")
